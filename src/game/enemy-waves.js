const EnemyWaveMethods={
  ...
  startWave(){if(this.ended||this._allLevelWavesSpawned)return;this.wActive=true;this._waveSpawnDone=false;const waveNo=this.wave,lw=this.levelWave();this._waveRoster=this.orderWaveRoster(lw?lw.roster.slice():this.buildWaveRoster());this.wC=this._waveRoster.length;this.wS=0;this._spawnOffset=Phaser.Math.Between(0,MAP.spawns.length-1);this._waveNaturalDuration=this.naturalWaveDuration(this._waveRoster,lw,this._spawnOffset);this._waveSpawnElapsed=this.waveSpawnDelay(Math.max(0,this.wC-1));this._wt=this.time.addEvent({delay:500,loop:true,callback:()=>{if(this.wS>=this.wC){this._wt.remove();this.finishWaveSpawning();return}const si=this.activeSpawnLane(this.wS,lw,this._spawnOffset),ty=this._waveRoster[this.wS]||'E1';this.spawnE(si,ty,waveNo);this.wS++;if(this.wS>=this.wC){this._wt.remove();this.finishWaveSpawning()}}})},
